...
3. Hash that refetched description (it has no footer yet) deterministically in the shell — never estimate it. **Hash the exact description string returned by the Step-2 `Linear:get_issue` refetch** — write *those bytes* verbatim to the temp file, or pipe them straight to `sha256sum` — never a reconstructed, hand-normalized, or normalization-predicted version of the body. The refetch exists precisely to capture Linear's actual normalization; re-deriving the body by hand defeats it, and a single mispredicted change (`_`→`*`, autolink, entity-escaping) yields a footer hash that never matches on re-run, silently breaking idempotency. For example, write the refetched description to a temp file and run `sha256sum <file> | cut -c1-12`.
